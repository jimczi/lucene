...
public class OneMergeWrappingMergePolicy extends FilterMergePolicy {
  ...
  /**
   * Wrapping creates a new {@link OneMerge}, so any behaviour not forwarded is lost. Losing
   * partitioning would silently produce a single output instead of several, so it is checked rather
   * than left to be discovered as missing data. Extend {@link MergePolicy.FilterOneMerge} to
