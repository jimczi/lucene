...
public class NeighborArray {
  ...
  public void addAndEnsureDiversity(
      int newNode, float newScore, int nodeId, RandomVectorScorer scorer) throws IOException {
    addOutOfOrder(newNode, newScore);
    if (size < nodes.length) {
      return;
    }
    // we're oversize, need to do diversity check and pop out the least diverse neighbour
    removeIndex(findWorstNonDiverse(nodeId, scorer));
   ...
   * neighbours
   */
  private int findWorstNonDiverse(int nodeOrd, RandomVectorScorer scorer) throws IOException {
    scorer.setQueryOrd(nodeOrd);
  ...
  private boolean isWorstNonDiverse(
    ...
    scorer.setQueryOrd(nodes[candidateIndex]);
